Feature: Pubilc user can select a calendar by its attributes
	...
	As a 'Public User'
	I want to be able to select a calendar by building, type, time, dept
	so that I can have options in my reservations

Background: calendars have been added to the database

	Given the following calendars exist:
	|name           |key |visib  | fee_required |disabled |building  |type        |dept     |
...
Scenario: able to filter calendars by type
	When I try to search by type with "personal"
